...
拾光排版器把公众号写作里反复调整字号、间距、配色和组件的工作，整理成一个本地优先工具。普通用户推荐直接使用在线版；需要断网使用或长期保留工具文件时，再下载 HTML 离线打开。两种方式都不需要注册账号，文章默认只保存在当前浏览器。

## 立即使用

- **推荐主入口：** [立即在线使用](https://xiangxiuchen.github.io/shiguang-wechat-typesetter/app/)——打开即用，同一地址持续获得最新版。
- **离线备用：** [下载固定最新 ZIP](https://github.com/xiangxiuchen/shiguang-wechat-typesetter/releases/latest/download/shiguang-wechat-typesetter-free.zip)——适合断网使用或长期留存，旧文件不会自动升级。
- **单 HTML 备用：** [下载固定最新 HTML](https://github.com/xiangxiuchen/shiguang-wechat-typesetter/releases/latest/download/shiguang-wechat-typesetter-latest.html)
- **示例项目：** [下载可导入示例](examples/shiguang-launch-example.json)

在线使用不等于云同步。草稿保存在当前浏览器；换电脑、换浏览器或清理浏览器数据前，请先下载文章备份 JSON。

## 三步完成一次排版

1. **粘贴全文并排版：** 复制整篇文章，点击“① 粘贴全文并排版”；也可以手动粘贴或选择 `.md` 文件。
2. **生成排版：** 第一次保持推荐设置，点击“② 生成排版”。工具会整理普通正文、代码、公式、链接、图片和表格；示例不用手动删除，新稿会自动替换。
3. **检查并复制：** 按需切换“排版样式（不改正文）”，再点击“③ 检查并复制到微信”。发现内容阻断项时工具不会继续复制。
...
- **容易备份：** 支持本地草稿、快照和文章备份 JSON 导入导出。
- **发布更稳：** 编辑预览和微信发布输出分开，复制前经过专用清洁与预检。
- **同一地址持续更新：** 在线版地址不变；离线版可检查新版，但不会自动覆盖本地文件。

## 三种文件不要混淆

- **排版器 HTML／ZIP：** 工具本身。在线版更新后直接刷新使用；离线版需要主动下载新版。
- **文章备份 JSON：** 保存文章、区块和主题，用于换浏览器、换电脑或升级离线版。
- **微信复制页 HTML：** 自动复制失败时的备用发布页面，不是文章备份。

离线版升级时，请先下载文章备份 JSON，再打开新版并选择恢复文章备份；核对标题、区块数和主题后，再决定是否删除旧 HTML。
...
核心排版可以断网使用，文章和草稿保存在你自己的浏览器中，不会在在线版和离线版之间自动同步。V4.1.6 起，联网时每 24 小时最多读取一次公开版本号，请求不包含正文、标题或草稿；失败时不影响编辑。GitHub Pages 页面和版本请求的基础访问数据适用 GitHub 的隐私规则。详细说明见 [PRIVACY.md](PRIVACY.md)。
